...
try{
    ...
    $objModelo->nombre_usuario = (isset($datosRecibidos['nombre_usuario'])) ?  $datosRecibidos['nombre_usuario'] : '';
    $objModelo->apellido_usuario = (isset($datosRecibidos['apellido_usuario'])) ?  $datosRecibidos['apellido_usuario'] : '';
    $objModelo->email_usuario = (isset($datosRecibidos['correo_usuario'])) ?  $datosRecibidos['correo_usuario'] : '';
    ...
    $objModelo->confirm_pass = (isset($datosRecibidos['confirmacion_pass'])) ?  $datosRecibidos['confirmacion_pass'] : '';
    $objModelo->terminos = (isset($datosRecibidos['aceptacion_termino'])) ?  $datosRecibidos['aceptacion_termino'] : '';

    //CONVERTIR DATOS HTML SCRIPT A TEXTO
    $objModelo->id_usuario = htmlspecialchars($objModelo->id_usuario);
    $objModelo->nombre_usuario = htmlspecialchars($objModelo->nombre_usuario);
    $objModelo->apellido_usuario = htmlspecialchars($objModelo->apellido_usuario);
    $objModelo->email_usuario = htmlspecialchars($objModelo->email_usuario);
    ...
    if(strtolower($metodo) == "registrar" || strtolower($metodo) == "actualizar"){
        ...
        if(empty($objModelo->nombre_usuario)){
            ...
            exit;
        }else if(empty($objModelo->confirm_pass)){
            echo json_encode(["success" => "false", "mensaje" => "Favor de Ingresar una Confirmación de Contraseña", "error" => "Datos Faltantes", "datos" => []]);
            exit;
        }else if($objModelo->confirm_pass != $objModelo->pass_usuario){
            echo json_encode(["success" => "false", "mensaje" => "La Contraseña no es igual a la Confirmación", "error" => "Datos Faltantes", "datos" => []]);
            exit;
        }
        else if(empty($objModelo->terminos) || $objModelo->terminos == false){
            echo json_encode(["success" => "false", "mensaje" => "Favor de Aceptar los Terminos y Condiciones", "error" => "Datos Faltantes", "datos" => []]);
            exit;
        }
    ...
    if (!preg_match($regex, $objModelo->nombre_usuario)) {
        ...
        exit;
    }else if (!preg_match($regex, $objModelo->confirm_pass)) {
        echo json_encode(["success" => "false", "mensaje" => "La Confirmación Contiene Caracteres no Válidos", "error" => "Datos Faltantes", "datos" => []]);
        exit;
    }
        ...
        switch (strtolower($metodo)) { 
            case 'registrar':
                ...
                $objController->insertarUsuario($objModelo);
                break;
