...
class UsuariosVideosController {
    ...
    public function __construct($db) {
        $this->modelo = new UsuariosVideos($db);
    }

    public function obtenerTodosPorId($usuario) {
        $modelo = $this->modelo->obtenerTodosPorId($usuario);
        if(!empty($modelo)){
    ...
    public function insertarVideo(UsuariosVideos $data) {

        try {
            $modelo = $this->modelo->crearVideos($data);
            if(is_array($modelo) && array_key_exists('error', $modelo)){                      
                if(isset($modelo['error']) && $modelo['error'] === true){
                    if(is_array($modelo) && array_key_exists('mensaje', $modelo)){
            ...
            if(is_array($modelo) && array_key_exists('success', $modelo)){
                if (isset($modelo['success']) && $modelo['success'] === true) {
                    ...
                    if (isset($modelo['duplicado']) && $modelo['duplicado'] === true) {
                        echo json_encode(["success" => "false", "mensaje" => "El Registro ya esta Registrado", "datos" => []]);
                    }else if (isset($modelo['existe']) && $modelo['existe'] === true) {
    ...
    public function actualizarVideo(UsuariosVideos $data) {

        try {

            $modelo = $this->modelo->actualizarVideos($data);
    ...
    public function eliminarVideo($id) {
        if ($this->modelo->eliminarVideo($id)) {
    ...
    public function obtenerPorIdVideo(UsuariosVideos $id) {
        $modelo = $this->modelo->obtenerPorIdVideo($id);
        if(!empty($modelo)){
            if(is_array($modelo) && array_key_exists('success', $modelo)){
                if(isset($modelo['success']) && $modelo['success'] === true){
                    echo json_encode(["success" => "true", "mensaje" => "Datos Econtrado", "datos" => $modelo]);
